...
#第三个为相似离度
##################
uk.result<-function(map.sp, Test.spdf, df.sp, n, nmax=15, nmin=10, beta=0.5){
    var.pred<-NULL
    degAD.df<-NULL
    for(i in 1:n) {
        dfx<-df.sp[,c(i+5,3:4)]
        var.formula=DF2formula(dfx@data)  #var(i)~lat+lon     +hh
        vgmodel<-autofitVariogram(formula=var.formula, input_data=dfx, model = c("Sph", "Exp", "Gau", "Log", "Ste", "Wav"))
        vgmodel<-vgmodel$var_model
        uk<-krige(formula=var.formula, model=vgmodel, locations=dfx, newdata=map.sp, nmax=nmax, nmin=nmin)  #克里格内插
        #提取最邻近点
        uk.pred<-pointDistance(uk, Test.spdf) %>%
           apply(.,2, which.min) %>%  #最邻近的1个点，因为网格经纬度坐标有舍入等差异
           uk[. ,]   #提取对应点的插值
        var.pred<-cbind(var.pred, uk.pred@data$var1.pred)

...
#第三个为相似离度
##################
g_knn.result<-function(map.raster, Test.spdf, df.sp, n, nmax=6,idpx=5,beta=0.5){
    var.pred<-NULL
    degAD.df<-NULL
    ...
    for(i in 1:n) {
        dfx<-df.sp[,i+5]
        colnames(dfx@data)<- "x"
        gknn<-gstat(formula=var.formula, locations=dfx,nmax=nmax, set=list(idp=idpx)) %>%
           raster::interpolate(map.raster, . ) %>%
           mask(. , Test.spdf) %>%  #掩膜提取
           rasterToPoints(. , spatial=T)    
        var.pred<-cbind(var.pred, gknn@data$var1.pred)
